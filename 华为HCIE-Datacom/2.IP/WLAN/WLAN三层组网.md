...
| 维度         | 二层组网            | 三层组网            |
| ---------- | --------------- | --------------- |
| 业务 VLAN 规划 | 所有 AP 归属同一 VLAN | AP 可归属不同 VLAN   |
| 支持的漫游类型    | 仅二层漫游           | 二层漫游和三层漫游       |
| IP 地址变化    | 不变              | 可能变化（三层漫游时）     |
| 典型场景       | 小型园区、单一办公区域     | 大型园区、跨楼栋 / 区域部署 |

**LSW1作为AP的网关，通过vlan 50通信LSW1连接AC通过VLAN 100三层通信**
![400](assets/WLAN三层组网/file-20251207204857911.png)
