...
![900](assets/11、IPv4与IPv6的比较(自研)/file-20251208202704341.png)/file-20251208202704341.png)

**IPv6基本报头也称之为固定报头。**
固定报头包含8个字段，总长度为40Byte。这8个字段分别为：版本（Version）、流量类型（Traffic Class）、流标签（Flow Label）、净荷长度（Payload Length）、下一个报头（Next Header）、跳数限制（Hop Limit）、源IPv6地址、目的IPv6地址。  
1.版本（Version）  
...
8.目的地址（Destination Address）
	表示接收方的地址，长度为128bit。
![](assets/11、IPv4与IPv6的比较(自研)/file-20251208202807708.png)
![](assets/11、IPv4与IPv6的比较(自研)/file-20251208202819595.png)
...
- 增加流标签：提高QoS效率。
![](assets/11、IPv4与IPv6的比较(自研)/file-20251208202838538.png)/file-20251208202838538.png)