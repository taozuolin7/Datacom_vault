...
队列的名称和DSCP关键字表示的名称，没有任何关系  
	1.EF(Expedited Forwarding（加速转发）)队列：满足低延迟的业务，分配的带宽是最大值的带宽（最多可以使用的带宽值）  
		提供给实时性业务的，提供低时延的服务  
		LLQ队列：特殊的EF队列  
		提供给更低延迟的业务，建议给语音业务使用  
	2.AF队列(Assured Forwarding（确保转发）)：满足带宽保障的业务，分配的带宽是最小值的带宽（最少可以使用的带宽值）  
		提供给数据性的业务，提供带宽保障的服务  
	3.BE队列Best Effort（尽力而为）：给不需要qos保障的业务使用，执行尽力而为的服务  
...
#### AR4和AR5通过复杂流分类  
![](assets/2、为什么要有qos？/file-20251209234226350.png)
将访问PC1的流量  
AR4标记为802.1p优先级值4  
AR5标记为802.1p优先级值6
```
...
![](assets/2、为什么要有qos？/file-20251209234226349.png)
AR1通过qos的映射表将  
