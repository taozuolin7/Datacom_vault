...
.。。。。。  
如果连续三次计算的路由时间都大于 设置的最大时间，则时间清空 从第一次路由计算的时间重新叠加￼￼快速重路由:提高网络故障时的恢复速度  
[AR1]ospf  
[AR1-ospf-1]frr  
[AR1-ospf-1-frr]loop-free-alternate 启用无环路备用路径￼通过预先计算并维护一条无环路的备用路径，当主路径失效时立即切换，避免路由环路和流量中断
...
[AR4-ospf-1]stub-router on-startup \<5-65535\>s  
