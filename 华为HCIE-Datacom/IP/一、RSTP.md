...
![Exported image](Exported%20image%2020251206164728-0.octet-stream)

```
   
￼RSTP对于STP的改进：  
1.端口角色  
   1.STP：DP 指定端口  RP 根端口  
   2.RSTP：DP 指定端口 RP 根端口  AP 预备端口  BP 备份端口  
   AP端口作为RP端口的备份、BP端口作为DP端口的备份
```
 
```
2.端口状态  
   1.STP：disable、blocking、listening、learning、forwarding  
   2.RSTP：discarding、learning、forwarding  
   将三种不转发业务数据的状态变为了一种状态
```
 
```
3.BPDU类型：  
   1.STP：配置BPDU、TCN BPDU  
           1.type字段描述为0，表示BPDU是STP的配置BPDU  
           2.flag字段（TC bit、TCA bit）  
   2.RSTP：RST BPDU（配置BPDU）  
...
4.BPDU报文的处理：  
   1.BPDU报文的发送者  
      1.STP：拓扑稳定后，只有根桥发送BPDU，其他非根桥设备转发BPDU  
      2.RSTP：所有设备发送自身的BPDU（就算没有收到根桥发送的BPDU，也会按照2s的时间发送BPDU）  
   2.更短的BPDU超时时间  
      1.STP：20s的BPDU老化  
      2.RSTP：默认18s的老化 可以调整为6s     [HUAWEI] stp timer-factor \<1-10\>  
   3.次级BPDU的处理  
      1.STP：需要等待接口的BPDU老化再进行处理  
      2.RSTP：触发次级BPDU的处理机制，直接处理（直接向对端发送更好的BPDU）
```
...
```
   3.P/A机制￼*P/A机制要求两台交换设备之间链路必须是点对点（P2P）的全双工模式  
...
![Exported image](Exported%20image%2020251206164730-1.png) ![Exported image](Exported%20image%2020251206164732-2.png)