...
  bestroute nexthop-priority ipv6 high-level
  # 双栈共存场景，可能存在相同前缀、不同下一跳类型的两条路由，其中1条路由下一跳是IPv4的，1条路由是IPv6的，通过配置**bestroute nexthop-priority ipv6 high-level**命令可以控制下一跳IPv6路由优于下一跳IPv4的路由，通过配置**bestroute nexthop-priority ipv4 high-level**命令可以控制下一跳IPv4路由优于下一跳IPv6的路由。 #  命令用来配置BGP学习的路由基于下一跳选路IPv4优先级高还是IPv6优先级高。选路顺序在比较IGP Metric前面
```

采集脚本：
```R
display bgp evpn route-distinguisher 218.201.3.37:90 routing-table prefix-route
# 查看RD值位218.201.3.37:90这个vpn实例的evpn前缀路由信息
display ip routing-table vpn-instance IMS-GROUPVOIP | in 2409:8860
# 查看IMS-GROUPVOIP从RR那里学习来的ip路由表
```
